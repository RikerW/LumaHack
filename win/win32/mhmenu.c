/* LumaHack 3.6	mhmenu.c	$NHDT-Date: 1432512811 2015/05/25 00:13:31 $  $NHDT-Branch: master $:$NHDT-Revision: 1.48 $ */
...
typedef struct mswin_lumahack_menu_window {
                              ...
                              BOOL activate)
{
    ...
    *_selected = NULL;
    ...
    data->is_active = activate && !GetNHApp()->regLumaHackMode;
    ...
    if (NHMENU_HAS_GLYPH(*item)) {
        if (!IS_MAP_ASCII(iflags.wc_map_mode)) {
            ...
            if (GetNHApp()->bmpMapTiles == GetNHApp()->bmpTiles) {
                /* using original lumahack tiles - apply image transparently */
...
onListChar(HWND hWnd, HWND hwndList, WORD ch)
{
    ...
    case ' ': {
        if (GetNHApp()->regLumaHackMode) {
            /* LumaHack mode: Scroll down one page,
