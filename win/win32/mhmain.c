/* LumaHack 3.6	mhmain.c	$NHDT-Date: 1432512811 2015/05/25 00:13:31 $  $NHDT-Branch: master $:$NHDT-Revision: 1.62 $ */
...
typedef struct mswin_lumahack_main_window {
...
MainWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_KEYDOWN: {
        ...
        /* translate arrow keys into lumahack commands */
        switch (wParam) {
        case VK_LEFT:
    ...
    {
        /*
          If not lumahackmode, don't handle Alt-keys here.
          If no Alt-key pressed it can never be an extended command
        */
        if (GetNHApp()->regLumaHackMode && ((lParam & 1 << 29) != 0)) {
    ...
    case WM_CLOSE: {
        ...
        if (program_state.gameover) {
            ...
            case IDYES:
                ...
                return control to LumaHack core at this point */
                if (IsWindow(GetNHApp()->hPopupWnd))
                    SendMessage(GetNHApp()->hPopupWnd, WM_COMMAND, IDCANCEL,
                                0);

                /* tell LumaHack core that "hangup" is requested */
                hangup(1);
#else
                NHEVENT_KBD('y');
                dosave();
...
onWMCommand(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
    ...
    case IDM_NHMODE: {
        GetNHApp()->regLumaHackMode = GetNHApp()->regLumaHackMode ? 0 : 1;
...
mswin_apply_window_style(HWND hwnd) {
    ...
    } else if (GetNHApp()->regLumaHackMode) {
        /* do away borders */
