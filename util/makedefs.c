/* LumaHack 3.6  makedefs.c  $NHDT-Date: 1582403492 2020/02/22 20:31:32 $  $NHDT-Branch: LumaHack-3.7 $:$NHDT-Revision: 1.177 $ */
    ...
    while (*options) {
        ...
        case 'S':
            ...
            do_rnd_access_file(BOGUSMONFILE,
                /* default bogusmon:  iconic monster that isn't in lumahack */
                               "grue");
            break;
        case 'h':
        case 'H':
...
const char *build_date;
{
    ...
    Sprintf(outbuf, "%s LumaHack%s Version %s%s - last %s %s.", PORT_ID,
...
#if 0
    Sprintf(outbuf, "%s LumaHack%s %s Copyright 1985-%s (built %s)",
...
do_date()
{
...
#ifdef AMIGA
    {
        ...
        Fprintf(ofp, "\"\\0$VER: LumaHack %d.%d.%d (%d.%d.%d)\"\n",
...
#endif
#endif
    /* pattern matching method will be substituted by lumahack at run time */
...
do_permonst()
{
    ...
    if (use_enum) {
        Fprintf(ofp, "\nenum monnums {");
#if 0
        /* need #define ENUM_PM for the full LumaHack build to include these */
        Fprintf(ofp, "\n        NON_PM = -1,");
        Fprintf(ofp, "\n        LOW_PM = 0,");
#endif
    }
...
/* check %E record for "[summary text]" that lumahack can stuff into the
