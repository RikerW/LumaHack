/* LumaHack 3.6	config.h	$NHDT-Date: 1575245033 2019/12/02 00:03:53 $  $NHDT-Branch: LumaHack-3.6 $:$NHDT-Revision: 1.126 $ */
...
#define HACKDIR "/boot/apps/LumaHack"
#endif
#endif
 ...
 * format loaded by LumaHack code, or (2) using the XPM format loaded by
 ...
 *              MAXPLAYERS   (see MAX_NR_OF_PLAYERS below and lumahack.sh)
 ...
 *            Regular lumahack options can also be specified in order to
 *            provide system-wide default values local to your system:
 *              OPTIONS      (same as in users' .lumahackrc or defaults.nh)
 *
 ...
 *              default is 'sysconf' in lumahack's playground.
 */
 ...
 *      that gets written to the save file by LumaHack, in contrast
 *      to COMPRESS or ZLIB_COMP which compress the entire file after
 *      the LumaHack data is written out.
 *
 ...
 *      defined, LumaHack can read an rlecomp or zerocomp savefile in, yet
 ...
 *      date+time in include/date.h (to be shown by lumahack's 'v' command)
 ...
 *      and the value should be in UTC rather than local time.  LumaHack
 ...
 */
#ifndef HACKDIR
#define HACKDIR "/usr/games/lib/lumahackdir"
#endif
...
/*
 * There are various choices for the LumaHack vision system.  There is a
 ...
 *  ESC [ 1 ; 3 z           End of data. LumaHack has finished sending data,
 *                          and is waiting for input.
 *
 * Whenever LumaHack outputs anything, it will first output the "select window"
 * code. Whenever LumaHack outputs a tile, it will first output the "start
 * glyph" code, then the escape codes for color and the glyph character
 * itself, and then the "end glyph" code.
 *
 * To compile LumaHack with this, add tile.c to WINSRC and tile.o to WINOBJ
...
/* LumaHack will execute an external program whenever a new message-window
...
#ifndef DUMPLOG_FILE
#define DUMPLOG_FILE        "/tmp/lumahack.%n.%d.log"
